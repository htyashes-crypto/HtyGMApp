...
| 服务端→客户端 | `command-list` | `{ commands: GmCommandMeta[] }` | 命令清单（一次性，命令少时用） |
| 服务端→客户端 | `command-list-begin` | `{ total }` | （可选流式）下发开始 |
| 服务端→客户端 | `command-list-chunk` | `{ commands, index }` | （可选流式）下发一批命令 |
| 服务端→客户端 | `command-list-end` | (无) | （可选流式）下发结束 |
...
> **命令清单：一次性 vs 流式（二选一，GMApp 都支持、无需协商）**
> 服务端收到 `list-commands` 后，回 **一条** `command-list`（简单，命令少时用）；**或** 回 **一组** `command-list-begin{total}` → 多个 `command-list-chunk{commands,index}` → `command-list-end`（流式，命令多时用）。
> 流式适用于命令多、每条带图标的场景——把收集/编码分摊到多帧（如每帧 6 条），避免一次性收集编码卡住服务端主线程；GMApp 据 `begin.total` 显示初始化进度条、按 `chunk` 累积推进、`end` 结束。两种发哪个 GMApp 自动识别。
...
```ts
{
  ...
  enumOptions?: string[]        // 枚举可选项的【实际执行值】（仅 type='Enum'；下拉 value、回传值都用它）
  enumDisplayNames?: string[]   // 可选，与 enumOptions 一一对齐的显示名；非空时下拉显示它、value 仍用 enumOptions（实际值常是短写/英文，显示名给测试人员看）
  enumAsInt?: boolean           // 可选，true 时该枚举映射为整数：enumOptions 填整数字符串(如 "0"/"1")、enumDisplayNames 填可读名(如 关闭/开启)；GMApp 仍下拉选择，但回传时 args[].type 用 "Int"(而非 "Enum")，服务端按整数解析执行
  defaultValue?: string         // 默认值（可选，字符串形式；Enum 时是实际值）
}
```
